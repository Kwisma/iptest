...
class CSVProcessor {
  ...
  async process() {
    try {
      ...
      const txtUnlimitedFilePath = this.getFilePath(this.config.targetFile.replace(".csv", "_unlimited.txt"));
      const txtLimitedFilePath = this.getFilePath(this.config.targetFile.replace(".csv", "_limited.txt"));
  ...
  filterCountriesByMinCount(groupedEntries) {
    ...
    return Object.fromEntries(
      Object.entries(groupedEntries).filter(([country, entries]) => 
  ...
  async generateAndSaveResults(groupedEntries, txtUnlimitedFilePath, txtLimitedFilePath) {
    ...
    console.log(`所有国家: ${allCountries.join("、")} (共 ${allCountries.length} 个国家)`);
    console.log(`符合条件国家 (记录数 >= ${this.config.perCountryCount}): ${filteredCountries.join("、")} (共 ${filteredCountries.length} 个国家)`);
