...
function createWebSocketConnection(ip, port, location, testRound) {
  ...
  console.log(`🔄 开始测试 ${location} (${ip}:${port}) 第${testRound}/${TESTS_PER_IP}次 [活跃: ${activeConnections}, 已完成: ${completedTests}, 总测试数: ${ipPortList.length * TESTS_PER_IP}]`);
  ...
  ws.on('message', (data) => {
    ...
    console.log(`📨 已接收数据 from ${location} 第${testRound}次: ${data.toString()}`);
  ...
  ws.on('error', (error) => {
    console.log(`❌ 连接错误 ${location} (${ip}:${port}) 第${testRound}次: ${error.message}`);
    cleanup();
    if (!testCompleted) {
      testCompleted = true;
      handleTestCompletion(ip, port, location, false, testRound);
...
function handleTestCompletion(ip, port, location, success, testRound, latency) {
  ...
  if (success) {
    ...
    console.log(`✅ 第${testRound}次测试成功 (${result.successes}/${TESTS_PER_IP} 成功)`);
  } else {
    result.failures++;
    console.log(`❌ 第${testRound}次测试失败 (${result.failures}/${TESTS_PER_IP} 失败)`);
  }
  
  activeConnections--;
  completedTests++;
  
  console.log(`📊 进度: ${completedTests}/${ipPortList.length * TESTS_PER_IP} 次测试 (活跃: ${activeConnections})`);
  
  // 检查是否所有测试已完成
  if ((result.successes + result.failures) === TESTS_PER_IP) {
    result.completed = true;
    if (result.successes === TESTS_PER_IP) {
      console.log(`🎉 ${location} (${ip}:${port}) 全部${TESTS_PER_IP}次测试通过！平均延迟: ${calculateAverage(result.latencies)}ms`);
    } else {
      console.log(`❌ ${location} (${ip}:${port}) 测试未通过 (成功: ${result.successes}/${TESTS_PER_IP})`);
    }
  }
...
function startNextTest() {
  ...
  while (activeConnections < MAX_CONCURRENT && nextTestIndex < ipPortList.length) {
    ...
    const result = getTestResult(nextIpPort.ip, nextIpPort.port, nextIpPort.location);
    
    // 检查这个IP是否已经开始测试
    if ((result.successes + result.failures) === 0) {
      // 第一次启动这个IP的测试
      createWebSocketConnection(nextIpPort.ip, nextIpPort.port, nextIpPort.location, 1);
    }
    nextTestIndex++;
  }
}
...
function startConcurrentTests() {
  console.log(`🚀 开始并发测试，每个IP测试${TESTS_PER_IP}次，最大并发数: ${MAX_CONCURRENT}`);
  console.log('📋 使用配置:', JSON.stringify(vlessConfig, null, 2));
...
function saveResults() {
  ...
  sortedCountries.forEach(country => {
    ...
    countryItems.forEach(item => {
      allData.push(`${item.ip}:${item.port}#${country}${countryCounters[country]}`);
    ...
    countryItems.forEach((item, index) => {
      if (index < 5) {
        top5Data.push(`${item.ip}:${item.port}#${country}${top5Counters[country]}`);
  ...
  console.log(`✅ 已保存每个国家前5个到 vless_top5.txt (${top5Data.length} 个)`);
...
function main() {
  ...
  console.log(`📋 每个IP测试 ${TESTS_PER_IP} 次，总共 ${ipPortList.length * TESTS_PER_IP} 次测试`);
