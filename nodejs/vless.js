...
function testConnection(target, testRound) {
  ...
  debug(`${COLORS.dim}开始测试 [${COLORS.cyan}${location}${COLORS.dim}] ${COLORS.white}${ip}:${port}${COLORS.dim} (${testRound}/${TEST_CONFIG.TESTS_PER_IP})${COLORS.reset}`, 
  ...
  const timeoutId = setTimeout(() => {
    if (!testCompleted) {
      debug(`${COLORS.brightYellow}连接超时${COLORS.reset} [${COLORS.cyan}${location}${COLORS.reset}]`, { time: `${TEST_CONFIG.CONNECTION_TIMEOUT}ms` });
  ...
  const completeTest = (success, details) => {
    ...
    if (success) {
      ...
      // 根据延迟选择颜色
      const latencyColor = getLatencyColor(details);
      ...
      info(
        ...
        `${roundStr} ` +
        `${latencyColor}${details}ms${COLORS.reset} ` +
        `${COLORS.dim}${timeSpent}ms${COLORS.reset}`
      );
    } else {
      result.failures++;
  ...
  ws.on("open", () => {
    debug(`${COLORS.green}连接建立${COLORS.reset} [${COLORS.cyan}${location}${COLORS.reset}]`, { time: `${Date.now() - startTime}ms` });
    handshakeSent = true;
    ws.send(generateVLESSHandshake());
  });
  ...
  ws.on("message", (data) => {
    ...
    if (chunk.length >= 2 && chunk[1] === 0) {
      ...
      completeTest(true, handshakeTime);
    }
  });
...
function saveResults() {
  ...
  // 失败记录
  results.filter(r => r.successes < TEST_CONFIG.TESTS_PER_IP)
  ...
  info(`${COLORS.green}已保存 ${COLORS.brightWhite}${detailedData.length}${COLORS.reset}${COLORS.green} 条详细记录到 vless_passed_detailed.txt${COLORS.reset}`);
  info(`${COLORS.green}已保存 ${COLORS.brightWhite}${failedData.length}${COLORS.reset}${COLORS.green} 条失败记录到 vless_failed.txt${COLORS.reset}`);
...
function main() {
  ...
  info(`${COLORS.white}配置: ${COLORS.brightYellow}并发=${TEST_CONFIG.MAX_CONCURRENT}${COLORS.reset}, ${COLORS.brightYellow}每IP测试=${TEST_CONFIG.TESTS_PER_IP}${COLORS.reset}次`);
